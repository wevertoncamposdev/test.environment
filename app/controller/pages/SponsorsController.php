...
use \app\dao\SponsorsDAO;
use \app\dao\OccupationDAO;
use \app\utils\UploadFiles;
use \app\view\View;
use stdClass;

class SponsorsController extends RenderPage
{
    ...
    public static function getTable($auth, $view)
    {
        $itens = '';
        $var = new stdClass();
        $query = SponsorsDAO::selectTable('', $view);

        if ($query->num_rows !== 0) {

            while ($row = $query->fetch_assoc()) {

                $var->id = $row['id_sponsors'];
                $var->name = $row['name_sponsors'];
                $var->description = $row['description_sponsors'];
                $var->image = $row['image_sponsors'];
                $var->start_date = $row['start_date_sponsors'];
                $var->end_date = $row['end_date_sponsors'];
                $var->date_post = $row['date_post_sponsors'];
                $var->date_update = $row['date_update_sponsors'];

                //VERIFICANDO STATUS DA PUBLICAÇÃO
                $row['status_sponsors'] == true ? $var->status = 'SIM':$var->status = 'NÃO';
                $row['public_status_sponsors'] == true ? $var->public_status = 'SIM':$var->public_status = 'NÃO';
    ...
    public static function insert($request, $auth, $path)
    {
        ...
        if ($resultfile->status === true) {
            ...
            $resultDB = SponsorsDAO::insert($post, $resultfile->path, $auth);
    ...
    public static function select($search, $auth)
    {
        ...
        $result = SponsorsDAO::select($search, $auth);
        return $result;
    }
    ...
    public static function put($request, $auth, $path)
    {
        ...
        if ($resultfile->status === true) {
            ...
            $resultDB = SponsorsDAO::put($post, $resultfile->path, $auth);
            $resultDB = json_decode($resultDB);
            if ($resultDB->status === true) {
    ...
    public static function delete($request, $auth, $path)
    {
        ...
        $resultDB = SponsorsDAO::delete($post->ID_UPDATE, $auth); //REMOVENDO BANCO DE DADOS
        $resultDB = json_decode($resultDB);
